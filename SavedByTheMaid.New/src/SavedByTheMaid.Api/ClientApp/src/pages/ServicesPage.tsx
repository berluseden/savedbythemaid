...
export default function ServicesPage() {
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            {services.map((service) => (
              <div
                ...
                className={`relative bg-white rounded-2xl shadow-sm border ${
                  service.popular ? 'border-brand ring-2 ring-brand' : 'border-gray-200'
                } p-6 hover:shadow-lg transition-shadow`}
              >
                {service.popular && (
                  <div className="absolute -top-3 left-6 bg-brand text-white text-xs font-semibold px-3 py-1 rounded-full">
                    Most Popular
                  </div>
                )}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {benefits.map((benefit, idx) => (
              <div key={idx} className="text-center">
                <div className="w-14 h-14 bg-accent-light/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
      ...
      {/* CTA Section */}
      <section className="py-16 bg-gradient-to-br from-accent-light/50 to-sky-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Link
            to="/booking"
            className="inline-flex items-center gap-2 bg-white text-brand px-8 py-4 rounded-xl font-semibold hover:bg-gray-100 transition-colors"
          >
