    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var usr = _context.Users.FirstOrDefault(u => u.UserName.Contains(model.Email));

                    if (usr == null)
                    {                        
                        var user = _userService.RegisterUser(userName: model.Email, isActive:true);
                        _pageCookieService.SetCookie("UserName", user.UserName, 100000);
                        _pageCookieService.SetCookie("UserId", user.UserId.ToString(), 100000);
                    }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {                   
                    var newUser = _userService.RegisterUser(userName: model.Email, isActive: true);

                    _pageCookieService.SetCookie("UserName", newUser.UserName, 100000);
                    _pageCookieService.SetCookie("UserId", newUser.UserId.ToString(), 100000);
